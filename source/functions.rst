...
我们将在\ :doc:`后面的章节 <./data_structures>`\ 探索有关元组与字典的更多内容。
...
函数的第一行逻辑行中的字符串是该函数的
*文档字符串（DocString）*\ 。这里要注意文档字符串也适用于后面相关章节将提到的\ :doc:`模块（Modules） <./modules>`\ 与\  :doc:`类（Class） <./object_oriented_programming>`\
。
...
将 *所有东西* 都视为一个对象，这其中自然包括函数。我们将在后面的\ :doc:`类（Class） <./object_oriented_programming>`\ 章节讨论有关对象的更多细节。
